...
Large files under `data/`, `models/`, and `test-videos/*.mp4` stay on disk and stay out of Git. **Do not delete `data/` because the live camera path does not read it.** Those folders are the training and R&D foundation for later models.

---

## Runtime

Used when running the API and dashboard.

| Path | Role |
|------|------|
| `test-videos/01_ppe/ppe_construction_site.mp4` | PPE 01 |
| `test-videos/02_person/zone_multi_person.mp4` | Person 02 |
| `test-videos/03_restricted_zone/worker_single_person.mp4` | Zone 03 and worker-idle 07 |
...
| `models/quality.pt` | MVTec good/defect classifier (case 05, after training) |
| `test-images/quality/` | Optional stills for case 05 (`good/` + `defect/`). Falls back to MVTec. |
| `data/factory.db` | SQLite event store for the API (`DB_PATH`) |

`models/experiments/` is disposable training output. Runtime needs the three PPE/person `.pt` files. Case 05 also needs `quality.pt`. `logs/` is scratch and can be emptied anytime.
...
| `data/quality/mvtec/` | [MVTec AD](https://www.mvtec.com/company/research/datasets/mvtec-ad) — trains `quality.pt` (still-image case 05) | No — trains `quality.pt` |
...
Kept on purpose. The live camera path does not read these folders.
