...
from app.core.bbox import containment, is_valid_bbox
from app.detectors.base import BaseDetector, DetectionResult
from app.detectors.tracker import ObjectTracker
...
class PPEDetector(BaseDetector):
    ...
    def _infer(self, frame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            drop = {"Person"}
            if self.mask_model is not None:
                # Dedicated mask model handles face masks; multi-class Mask is unreliable here
                drop |= {"Mask", "NO-Mask"}
            items = [i for i in items if i["label"] not in drop]
        ...
        if self.mask_model is not None:
            try:
                ...
                masks = self._boxes_to_dicts(mr, self.mask_model, allowed_labels={"Mask"})
